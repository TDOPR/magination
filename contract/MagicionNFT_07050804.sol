...
contract MultiMagicion is Ownable{
    ...
    function sendNodeMFSDividends(address userAddress, uint8 level, uint256 _mfsAmount) private  {
        ...
        uint256 superSUSameLevelBonus = sendMFSAmount.mul(10).div(10000);
        ...
        uint256 thirdRemainLevelBonus = sendMFSAmount.mul(6).div(100);
        uint256 forthRemainLevelBonus = sendMFSAmount.mul(7).div(100);
        uint256 superForthRemainLevelBonus = sendMFSAmount.mul(10).div(100);
        uint256 forthSuperRemainLevelBonus = sendMFSAmount.mul(70).div(10000);
        uint256 i = Team[recieveAddress].level;
        // if (i == 1; i <= 4; i++) {
        //     if (i = 1) {
        //         updateTeamAllowanceBalance(recieveAddress, beginnerLevelBonus);
        //         address nextRecieveAddress = findNodeReferrer(recieveAddress, i);
        //         if (i + 1 = 2)
        //     } else if (i = 2) {
        //         updateTeamAllowanceBalance(recieveAddress, middleLevelBonus);
        //         address nextRecieveAddress = findNodeReferrer(recieveAddress, i);
        //     } else if (i = 3) {
        //         updateTeamAllowanceBalance(recieveAddress, advanceLevelBonus);
        //         address nextRecieveAddress = findNodeReferrer(recieveAddress, i);
        //     } else if (i = 4) {
        //         updateTeamAllowanceBalance(recieveAddress, superLevelBonus);
        //         address nextRecieveAddress = findNodeReferrer(recieveAddress, i);
        //     }
        //     updateTeamAllowanceBalance(recieveAddress, beginnerLevelBonus);
        //     address nextRecieveAddress = findNodeReferrer(recieveAddress, i);
        //     if (i + 1; i <= 4; i++) {
        //         updateTeamAllowanceBalance(recieveAddress, middleRemainLevelBonus);
        //         nextRecieveAddress = findNodeReferrer(nextRecieveAddress, i);
        //         if (i + 1; i <= 4; i++) {
        //             updateTeamAllowanceBalance(recieveAddress, middleRemainLevelBonus);
        //             nextRecieveAddress = findNodeReferrer(nextRecieveAddress, i);
        //             if (i + 1; i <= 4; i++) {
        ...
        if (Team[recieveAddress].level == 1) {
                updateTeamAllowanceBalance(recieveAddress, beginnerLevelBonus);        
                address nextRecieveAddress = findNodeReferrer(recieveAddress, Team[recieveAddress].level);
                if (Team[nextRecieveAddress].level == 2) {                
                    updateTeamAllowanceBalance(nextRecieveAddress, middleRemainLevelBonus);
                    nextRecieveAddress = findNodeReferrer(nextRecieveAddress, Team[nextRecieveAddress].level);
                    if (Team[nextRecieveAddress].level == 3) {
                        updateTeamAllowanceBalance(nextRecieveAddress, middleRemainLevelBonus);
                        nextRecieveAddress = findNodeReferrer(nextRecieveAddress, Team[nextRecieveAddress].level);
                    } else if (Team[nextRecieveAddress].level == 4) {
                        updateTeamAllowanceBalance(nextRecieveAddress, forthRemainLevelBonus);
                        nextRecieveAddress = findNodeReferrer(nextRecieveAddress, Team[nextRecieveAddress].level);
                        if (Team[nextRecieveAddress].level == 4) {
                            updateTeamAllowanceBalance(nextRecieveAddress, forthSuperRemainLevelBonus);
                        }
                    }
                } else if (Team[nextRecieveAddress].level == 3) {
                    updateTeamAllowanceBalance(nextRecieveAddress, thirdRemainLevelBonus);
                    nextRecieveAddress = findNodeReferrer(nextRecieveAddress, Team[nextRecieveAddress].level);
                    if (Team[nextRecieveAddress].level == 4) {
                        updateTeamAllowanceBalance(nextRecieveAddress, superRemainLevelBonus);
                        nextRecieveAddress = findNodeReferrer(nextRecieveAddress, Team[nextRecieveAddress].level);
                        if (Team[nextRecieveAddress].level == 4) {
                            updateTeamAllowanceBalance(nextRecieveAddress, sameLevelBonus);
                        }
                    }
                    } else if (Team[nextRecieveAddress].level == 4) {
                        updateTeamAllowanceBalance(nextRecieveAddress, superForthRemainLevelBonus);
                        nextRecieveAddress = findNodeReferrer(nextRecieveAddress, Team[nextRecieveAddress].level);
                        if (Team[nextRecieveAddress].level == 4) {
                            updateTeamAllowanceBalance(nextRecieveAddress, superSUSameLevelBonus);
                        }
                    }
        } else if (Team[recieveAddress].level == 2) {
            updateTeamAllowanceBalance(recieveAddress, middleLevelBonus);
            nextRecieveAddress = findNodeReferrer(recieveAddress, Team[recieveAddress].level);
            if (Team[nextRecieveAddress].level == 3) {
                updateTeamAllowanceBalance(nextRecieveAddress, middleRemainLevelBonus);
                nextRecieveAddress = findNodeReferrer(nextRecieveAddress, Team[nextRecieveAddress].level);
                if (Team[nextRecieveAddress].level == 4) {
                ...
                if (Team[nextRecieveAddress].level == 4) {
                        ...
                        updateTeamAllowanceBalance(nextRecieveAddress, superLevelBonus);
                        nextRecieveAddress = findNodeReferrer(nextRecieveAddress, Team[nextRecieveAddress].level);
                        if (Team[nextRecieveAddress].level == 4) {
                            updateTeamAllowanceBalance(nextRecieveAddress, superSameLevelBonus);
                        }
        } else {
            return;
        }
