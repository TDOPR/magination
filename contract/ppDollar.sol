 ...
 * functions have pddollarn added to mitigate the well-known issues around setting
...
// File contracts/PDDollar.sol


pragma solidity 0.6.12;
contract PDDollar is ERC20Burnable, Operator {
    ...
    // Have the rewards pddollarn distributed to the pools
    ...
    /**
     * @notice Constructs the PDDOLLAR ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("pddollar", "PDDOLLAR") {
        // Mints 1 PDDOLLAR to contract creator for initial pool setup
    ...
    function _getPDDollarPrice() internal view returns (uint256 _pddollarPrice) {
        try IOracle(pddollarOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("PDDollar: failed to fetch PDDOLLAR price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _pddollarPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_pddollarPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPDDollarOracle(address _pddollarOracle) public onlyOperatorOrTaxOffice {
        require(_pddollarOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints PDDOLLAR to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of PDDOLLAR to mint to
     * @return whether the process has pddollarn done
     */
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPDDollarPrice = _getPDDollarPrice();
            currentTaxRate = _updateTaxRate(currentPDDollarPrice);
            if (currentPDDollarPrice < burnThreshold) {
                burnTax = true;
            }
        }
