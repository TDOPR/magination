...
contract MultiMagicion is Ownable{
    ...
    function buyNewNftBox(uint256 _amount, address referrer) public updateReward(msg.sender) checkStart {
        ...
        updateReferrerAndTeam(_addr, _amount);
    ...
    function updateReferrerAndTeam(address userAddress, uint256 _amount) private {
        uint8 count = 0;
        while (true) {
            if (userIsRegi[userAddress]) {
                if (count <= 20) {
                    ...
                    Team[userAddress].teamTotalValue += _amount;
        ...
        if (Team[recieveAddress].level == 1) {
                ...
                if (Team[nextRecieveAddress].level == 2) {           
                    ...
                    if (Team[nextRecieveAddress].level == 4) {
                        ...
                        nextRecieveAddress = findNodeReferrer(nextRecieveAddress, Team[nextRecieveAddress].level);
                        if (Team[nextRecieveAddress].level == 4) {
                            updateTeamAllowanceBalance(nextRecieveAddress, sendMFSAmount.mul(10).div(10000));
                        }
                    }
    ...
    function balanceof(address _addr) public returns (uint256, uint256) {
        uint256 betValue = deposits[_addr];
        uint256 pendingIssueValue = Team[_addr].totalMFSAmount - users[_addr].allBonus;
